...
int inference_yolov7_raw(rknn_app_context_t* app_ctx, uint8_t *image_ptr,  letterbox_t letter_box, object_detect_result_list* od_results) { 
    ...
    if ((!app_ctx) || !(image_ptr) || (!od_results))
    {
        return -1;
    }
    ...
    inputs[0].fmt = RKNN_TENSOR_NCHW; // Opencv gives NCHW from blobFromImage
    inputs[0].size = app_ctx->model_width * app_ctx->model_height * app_ctx->model_channel;
    inputs[0].buf = image_ptr;
    ...
    // Give outputs to the NPU to fill in once processing is done
