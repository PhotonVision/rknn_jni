...
#include <set>
#include <vector>
#include "yolov11/postprocess_v11.h"
...
int post_process_11(cv::Size modelSize, rknn_output *outputs, BOX_RECT *padding, float conf_threshold, float nms_threshold, detect_result_group_t *od_results, int numClasses, std::vector<rknn_tensor_attr> &output_attrs, bool is_quant, int n_outputs)
{
    ...
    od_results->results.reserve(OBJ_NUMB_MAX_SIZE_V11);
    for (int i = 0; i < validCount; ++i)
    {
        if (indexArray[i] == -1 || last_count >= OBJ_NUMB_MAX_SIZE_V11)
        {
            continue;
        }
        int n = indexArray[i];
