...
static unsigned char *load_data(std::FILE *fp, size_t ofst, size_t sz) {
  ...
  ret = std::fseek(fp, ofst, SEEK_SET);
  if (ret != 0) {
    std::printf("blob seek failure.\n");
    return NULL;
  }

  data = (unsigned char *)malloc(sz);
  if (data == NULL) {
    std::printf("buffer malloc failure.\n");
    return NULL;
  }
  ret = std::fread(data, 1, sz, fp);
  return data;
}
...
YoloModel::YoloModel(std::string modelPath, int num_classes_,
                     ModelVersion type_, int coreNumber)
  ...
  std::printf("sdk version: %s driver version: %s\n", version.api_version,
              version.drv_version);
  ...
  // input_attrs = (rknn_tensor_attr *)calloc(io_num.n_input,
  ...
  // output_attrs = (rknn_tensor_attr *)calloc(io_num.n_output,
  // sizeof(rknn_tensor_attr));
  ...
  std::printf("Model quantized? %s\n", is_quant ? "YES" : "NO");

  if (input_attrs[0].fmt == RKNN_TENSOR_NCHW) {
    std::printf("model is NCHW input fmt\n");
    channels = input_attrs[0].dims[1];
    int height = input_attrs[0].dims[2];
    int width = input_attrs[0].dims[3];
    modelSize = {width, height};
  } else {
    std::printf("model is NHWC input fmt\n");
    int height = input_attrs[0].dims[1];
    int width = input_attrs[0].dims[2];
    channels = input_attrs[0].dims[3];
    modelSize = {width, height};
  }
  std::printf("model input height=%d, width=%d, channel=%d\n", modelSize.width,
              modelSize.height, channels);
...
int YoloModel::changeCoreMask(int coreNumber) {
  ...
  case 210:
    ...
    break;
  default:
    throw std::runtime_error("invalid core selection! core selected: " +
                             coreNumber);
    break;
  }
...
detect_result_group_t YoloModel::forward(cv::Mat &orig_img,
  ...
  if (img_width != modelSize.width || img_height != modelSize.height) {
    ...
    if (ret != 0) {
      std::fprintf(stderr,
                   "resRGA_CURRENT_API_HEADER_VERSIONize with rga error\n");
    }
    /*********
    // opencv
  ...
  auto detections = this->postProcess(outputs, params, orig_img.size(),
                                      {scale_w, scale_h}, BOX_RECT{});
...
YoloV11Model::postProcess(std::vector<rknn_output> outputs,
